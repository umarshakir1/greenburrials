...
class THEMECOMPLETE_EPO_Display {
	 ...
	 * @var array<mixed>
	 */
	public $epo_internal_counter_check = [];
	 ...
	 * @var integer
	 */
	public $original_epo_internal_counter = 0;
	 ...
	 * @var integer
	 */
	public $current_product_id_to_be_displayed = 0;
	 ...
	 * @var array<mixed>
	 */
	public $current_product_id_to_be_displayed_check = [];
	...
	public function get_tm_validation_rules( $element ) {
		...
		if ( ! empty( $element['validation1'] ) && 'password' !== $element['validation1'] ) {
	...
	public function tm_epo_fields( $product_id = 0, $form_prefix = '', $is_from_shortcode = false, $dummy_prefix = false ) {
		...
		if ( ( ! empty( THEMECOMPLETE_EPO()->cart_edit_key ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'tm-edit' ) ) || ( ! empty( THEMECOMPLETE_EPO()->cart_edit_key ) && isset( $_REQUEST['update-composite'] ) ) ) {
			if ( isset( WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ] ) ) {
				$tmpost_data = WC()->cart->cart_contents[ THEMECOMPLETE_EPO()->cart_edit_key ]['tmpost_data'];
				if ( ! empty( $tmpost_data ) && is_array( $tmpost_data ) ) {
					$prefix        = 'sections_repeater_';
					$prefix_length = strlen( $prefix );

					foreach ( $tmpost_data as $key => $value ) {
	...
	public function get_builder_display( $post_id, $field, $where, $vars, $form_prefix = '', $dummy_prefix = false ) {
		...
		$columns['w33-3'] = [ 'tcwidth tcwidth-33-3', 33.3 ];
		...
		if ( isset( $field['sections'] ) && is_array( $field['sections'] ) ) {
			...
			$product = wc_get_product( absint( THEMECOMPLETE_EPO_WPML()->get_original_id( $post_id ) ) );
			...
			foreach ( $field['sections'] as $section ) {
				...
				$section['label_size'] = ! empty( THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_override_section_label_type', '2' ) ) ? THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_override_section_label_type', '2' ) : $section['label_size'];
				...
				$sections_repeater              = isset( $section['sections_repeater'] ) ? $section['sections_repeater'] : '';
				$sections_repeater_quantity     = isset( $section['sections_repeater_quantity'] ) ? $section['sections_repeater_quantity'] : '';
				$sections_repeater_min_rows     = isset( $section['sections_repeater_min_rows'] ) ? $section['sections_repeater_min_rows'] : '';
				$sections_repeater_max_rows     = isset( $section['sections_repeater_max_rows'] ) ? $section['sections_repeater_max_rows'] : '';
				$sections_repeater_button_label = isset( $section['sections_repeater_button_label'] ) ? $section['sections_repeater_button_label'] : '';
				...
				$section_args = [
					...
					'divider'                        => $divider,
					'sections_repeater'              => $sections_repeater,
					'sections_repeater_quantity'     => $sections_repeater_quantity,
					'sections_repeater_min_rows'     => $sections_repeater_min_rows,
					'sections_repeater_max_rows'     => $sections_repeater_max_rows,
					'sections_repeater_button_label' => $sections_repeater_button_label,
				];
				...
				$sections_repeater_name        = [ 0 ];
				$sections_repeater_posted_name = 'sections_repeater_' . str_replace( '.', '_', $section['sections_uniqid'] );
				if ( isset( $_REQUEST[ $sections_repeater_posted_name ] ) ) {
					$sections_repeater_name = sanitize_text_field( wp_unslash( $_REQUEST[ $sections_repeater_posted_name ] ) );
					if ( is_array( $sections_repeater_name ) ) {
						$sections_repeater_name = $sections_repeater_name[0];
					}
					$sections_repeater_name = intval( $sections_repeater_name );
					if ( $sections_repeater_name > 0 ) {
						$sections_repeater_name = range( 1, intval( $sections_repeater_name ) );
					} else {
						$sections_repeater_name = [ 0 ];
					}
				}

				$section_args['sections_repeater_name']        = $sections_repeater_name;
				$section_args['sections_repeater_posted_name'] = $sections_repeater_posted_name;

				foreach ( $sections_repeater_name as $sections_repeater_key => $sections_repeater_value ) {
					$section_args['sections_repeater_key']   = $sections_repeater_key;
					$section_args['sections_repeater_value'] = $sections_repeater_value;
					...
					if ( isset( $section['elements'] ) && is_array( $section['elements'] ) ) {
						...
						if ( '' !== $section['sections_slides'] && ( 'slider' === $section['sections_type'] || 'tabs' === $section['sections_type'] ) ) {
							...
							$sections_slides = array_filter(
								$sections_slides,
								function ( $value ) {
									return '0' !== $value;
								}
							);
							$sections_slides = array_values( $sections_slides );
							$use_slides      = true;
						...
						foreach ( $section['elements'] as $element ) {
							...
							$element['label_size'] = ! empty( THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_override_element_label_type', '3' ) ) ? THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_override_element_label_type', '3' ) : $element['label_size'];
							...
							if ( 0 && 'radio' === $element['type'] || 'checkbox' === $element['type'] ) {
								$args['extra_class']       = ( empty( $args['extra_class'] ) ? '' : $args['extra_class'] . ' ' ) . 'tc-expand';
								$args['element_data_attr'] = array_merge(
							...
							if ( 'product' === $element['type'] ) {
								...
								$args['extra_class'] = ( empty( $args['extra_class'] ) ? '' : $args['extra_class'] . ' ' ) . 'cpf-type-product-' . $layout_mode . ' cpf-type-product-mode-' . $element['mode'];
								...
								if ( empty( $element['show_image'] ) && empty( $element['show_title'] ) && empty( $element['show_price'] ) && empty( $element['show_description'] ) && empty( $element['show_meta'] ) ) {
									if ( isset( $element['quantity_min'] ) && isset( $element['quantity_max'] ) ) {
										if ( ! empty( $element['quantity_min'] ) || ! empty( $element['quantity_max'] ) ) {
											if ( $element['quantity_min'] === $element['quantity_max'] ) {
												$args['extra_class'] .= ' tc-hidden-container';
											}
										}
									}
								}
								if ( empty( $args['element_data_attr']['data-show-image'] ) &&
								empty( $args['element_data_attr']['data-show-title'] ) &&
								empty( $args['element_data_attr']['data-show-price'] ) &&
								empty( $args['element_data_attr']['data-show-description'] ) &&
								empty( $args['element_data_attr']['data-show-meta'] ) &&
								'single' === $args['element_data_attr']['data-product-layout-mode'] &&
								! empty( $disable_epo ) ) {
									$args['extra_class'] .= ' tm-hidden';
								}
								if ( 'product' !== $element['mode'] ) {
								...
								if ( isset( THEMECOMPLETE_EPO()->tm_builder_elements[ $element['type'] ] )
									...
									if ( 'post' === $element_object->is_post ) {
										...
										if ( 'single' === $element_object->type || 'multipleallsingle' === $element_object->type || 'multiplesingle' === $element_object->type || 'singlemultiple' === $element_object->type ) {
											...
											if ( count( $sections_repeater_name ) > 1 ) {
												if ( count( $get_posted_name ) > 1 ) {
													$get_posted_name = array_slice( $get_posted_name, $sections_repeater_key, 1, true );
												}
											}
											if ( ! isset( $get_posted_name[ $sections_repeater_key ] ) ) {
												$get_posted_name = [ $sections_repeater_key => '' ];
											}
											...
											if ( ! empty( $element['dnmpbq'] ) ) {
												$fieldtype .= ' tc-is-dnmpbq';
											}
											...
											foreach ( $get_posted_name as $get_posted_key => $get_posted_value ) {
												$html_name          = $posted_name . ( ( ! empty( $repeater ) || ! empty( $sections_repeater ) ) ? '[' . $get_posted_key . ']' : '' );
												$html_quantity_name = $posted_name . '_quantity' . ( ( ! empty( $repeater ) || ! empty( $sections_repeater ) ) ? '[' . $get_posted_key . ']' : '' );
												if ( 'singlemultiple' === $element_object->type ) {
													if ( ( empty( $repeater ) && empty( $sections_repeater ) ) ) {
													...
													$element_args = [
														...
														'tax_obj' => ! $is_cart_fee ? false : wp_json_encode(
															( [
																...
																'tax_class' => themecomplete_get_element_tax_class( $element, $product ),
																'tax_rate'  => $this->get_tax_rate( themecomplete_get_element_tax_class( $element, $product ) ),
															] )
														),
													];
											...
											foreach ( $get_posted_name as $get_posted_key => $get_posted_value ) {
												...
												foreach ( $element['options'] as $value => $label ) {
													...
													if ( ! empty( $element['dnmpbq'] ) || ! empty( $element['is_dnmpbq_multiple'][ $field_counter ] ) ) {
														$fieldtype .= ' tc-is-dnmpbq';
													}
	...
	public function get_normal_display( $local_price_array = [], $args = [], $form_prefix = null, $dummy_prefix = null ) {
		...
		if ( is_array( $local_price_array ) && count( $local_price_array ) > 0 ) {
			...
			$attributes      = themecomplete_get_attributes( absint( THEMECOMPLETE_EPO_WPML()->get_original_id( $product_id ) ), true );
			$wpml_attributes = themecomplete_get_attributes( $product_id, true );
								...
								case 'select':
									...
									if ( $_tems_to_use && is_array( $_tems_to_use ) ) {
										foreach ( $_tems_to_use as $trid => $term ) {
											...
											if ( $has_term ) {
												$wpml_term_id = THEMECOMPLETE_EPO_WPML()->is_active() && function_exists( 'wpml_object_id_filter' ) ? wpml_object_id_filter( $term->term_id, $attribute['name'], false ) : false;
												if ( $wpml_term_id ) {
