...
final class THEMECOMPLETE_EPO_API_Base {
	...
	/**
	 * Reorganizes data by grouping repeater fields by their sections and indexes while preserving normal fields
	 *
	 * This function processes an array of field data, identifying repeater sections and normal fields.
	 * Repeater fields are grouped by their section type and key ID, while normal fields maintain their
	 * original position in the data structure. The function restructures the data so that repeater
	 * fields from the same index across different sections are grouped together.
	 *
	 * @param array $data Associative array where keys are field identifiers and values are field data arrays.
	 *                    Field data arrays should contain:
	 *                    - For normal fields: 'type' and optionally other field properties
	 *                    - For repeater fields: 'sections_repeater' (string), 'key_id' (string|int), 'label' (string).
	 *
	 * @return array Reorganized data array with the following structure:
	 *               - Normal fields remain in their original positions
	 *               - Repeater fields are grouped by their index across different repeater sections
	 *
	 * @internal
	 * Processing steps:
	 * 1. Iterates through input data to identify repeater sections and normal fields
	 * 2. Tracks repeater fields by section type and key ID
	 * 3. Builds a result structure that groups repeater fields by index
	 * 4. Reconstructs the final data array preserving order with grouped repeaters
	 *
	 * Note: The function handles complex nested repeater structures and maintains
	 * the original data integrity while reorganizing the field order.
	 */
	public function reorganize_repeater_data( $data = [] ) {
		$result                 = [];
		$new_data               = [];
		$index                  = 0;
		$is_in_section_repeater = false;
		foreach ( $data as $key => $value ) {
			if ( $is_in_section_repeater ) {
				--$index;
			}
			if ( isset( $value['sections_repeater'] ) && isset( $value['key_id'] ) ) {
				$is_in_section_repeater = true;
			} else {
				$is_in_section_repeater = false;
			}
			if ( $is_in_section_repeater ) {
				$result[ $index ][ $value['sections_repeater'] ][ $value['key_id'] ][] = $key;
			} else {
				++$index;
				$result[ $index ] = [
					'type' => 'normal',
		...
		foreach ( $result as $key => $value ) {
			if ( isset( $value['type'] ) && 'normal' === $value['type'] ) {
				$new_data[ $value['key'] ] = $data[ $value['key'] ];
			} else {
				foreach ( $value as $section => $fields ) {
					foreach ( $fields as $fields_keys ) {
	...
	/**
	 * Filters and reorganizes an array of EPO (Extra Product Options) data.
	 *
	 * This method iterates through a given EPO array, generates unique identifiers for each entry
	 * (based on the section and optionally repeater-related fields), and then reorganizes the data
	 * using the `reorganize_repeater_data()` method. The result is a clean, indexed array of
	 * processed EPO elements.
	 *
	 * @param array $epos Optional. The array of EPO data to filter and reorganize. Each item is expected
	 *                    to be an associative array containing at least the 'section' key, and possibly
	 *                    'repeater', 'key_id', and 'keyvalue_id' keys.
	 *
	 * @return array The filtered and reorganized EPO data as a numerically indexed array.
	 */
	public function filter_data( $epos = [] ) {
		$filtered_array = [];
		$new_epos       = [];
		foreach ( $epos as $tmcp ) {
			if ( isset( $tmcp[0] ) && is_array( $tmcp[0] ) ) {
				$tmcp = $tmcp[0];
			}
			$id = $tmcp['section'];
			...
			if ( isset( $filtered_array[ $id ] ) ) {
				$id .= THEMECOMPLETE_EPO_HELPER()->tm_uniqid();
			}
			$filtered_array[ $id ] = $tmcp;
		}

		$filtered_array = $this->reorganize_repeater_data( $filtered_array );
		foreach ( $filtered_array as $epo ) {
			$new_epos[] = $epo;
		}
		return $new_epos;
	}
	...
	public function parse_epo_order_data( $epo = [], $args = [], $restrictions = true ) {
		...
		if ( $epo && is_array( $epo ) && isset( $epo['section'] ) ) {
			...
			if ( ( isset( $epo['repeater'] ) || isset( $epo['sections_repeater'] ) ) && isset( $epo['key_id'] ) ) {
			...
			if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_posted_dates_wp_format' ) && isset( $epo['element'] ) && 'date' === $epo['element']['type'] ) {
				if ( isset( $epo['format'] ) ) {
					$epo['value'] = THEMECOMPLETE_EPO_HELPER()->convert_posted_date_to_wp_date( $epo['value'], $epo['format'] );
				}
			}
