...
final class THEMECOMPLETE_Extra_Product_Options {
	...
	public function __construct() {
		...
		$this->cart_edit_key_var     = apply_filters( 'wc_epo_cart_edit_key_var', $this->cart_edit_key_var );
		$this->cart_edit_key_var_alt = apply_filters( 'wc_epo_cart_edit_key_var_alt', $this->cart_edit_key_var_alt );
		$this->cart_edit_key         = '';
	...
	public function add_plugin_actions() {
		...
		if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_global_show_radiobuttons_disabled_choice' ) ) {
			add_action( 'wc_epo_hide_radiobuttons_disabled_choices', '__return_false' );
		}
		if ( 'yes' === THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_global_show_checkboxes_disabled_choice' ) ) {
			add_action( 'wc_epo_hide_checkboxes_disabled_choices', '__return_false' );
		}
		...
		// Add custom links to the WP Admin Bar.
		add_action( 'admin_bar_menu', [ $this, 'admin_bar_menu' ], 999 );
	}

	/**
	 * Add “Generate product link” node to the front-end WP Admin Bar.
	 *
	 * Only runs when:
	 * - the plugin’s backend is enabled (checked via THEMECOMPLETE_EPO_ADMIN()->is_backend_enabled())
	 * - the current request is *not* inside wp-admin (is_admin() === false)
	 *
	 * @param WP_Admin_Bar $admin_bar The WP_Admin_Bar instance, supplied by the ‘admin_bar_menu’ hook.
	 *
	 * @return void
	 */
	public function admin_bar_menu( $admin_bar ) {
		if ( is_admin() || ! THEMECOMPLETE_EPO_ADMIN()->is_backend_enabled() || ! $this->wc_vars['is_product'] ) {
			return;
		}

		$admin_bar->add_node(
			[
				'id'    => 'generate-product-link',
				'title' => esc_html__( 'Generate product link', 'woocommerce-tm-extra-product-options' ),
				'href'  => '#',
				'meta'  => [ 'class' => 'generate-product-link' ],
			]
		);
	}
				...
				if ( isset( $tmcp_meta['tmfbuilder'] ) ) {
					...
					if ( is_array( $builder ) && count( $builder ) > 0 && isset( $builder['element_type'] ) && is_array( $builder['element_type'] ) && count( $builder['element_type'] ) > 0 ) {
						...
						for ( $_s = 0; $_s < $_sectionscount; $_s++ ) {
							...
							$sections_repeater              = $this->get_builder_element( 'sections_repeater', $builder, $current_builder, $_s, '', 'sections', '', $_sections_uniqid );
							$sections_repeater_quantity     = $this->get_builder_element( 'sections_repeater_quantity', $builder, $current_builder, $_s, '', 'sections', '', $_sections_uniqid );
							$sections_repeater_min_rows     = $this->get_builder_element( 'sections_repeater_min_rows', $builder, $current_builder, $_s, '', 'sections', '', $_sections_uniqid );
							$sections_repeater_max_rows     = $this->get_builder_element( 'sections_repeater_max_rows', $builder, $current_builder, $_s, '', 'sections', '', $_sections_uniqid );
							$sections_repeater_button_label = $this->get_builder_element( 'sections_repeater_button_label', $builder, $current_builder, $_s, '', 'sections', '', $_sections_uniqid );
							...
							if ( '' !== $global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] ) {
								$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] = THEMECOMPLETE_EPO_CONDITIONAL_LOGIC()->convert_rules( json_decode( wp_unslash( $global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] ) ) );
								$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] = THEMECOMPLETE_EPO_CONDITIONAL_LOGIC()->convert_rules( $global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] );
								$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] = wp_json_encode( $global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ]['sections_logicrules'] );
							}
							...
							for ( $k0 = $_helper_counter; $k0 < $_helper_counter + $_sections[ $_s ]; $k0++ ) {
								...
								if ( $element_object ) {
									...
									if ( $sections_repeater && in_array( $current_element, [ 'product', 'multiple_file_upload', 'template', 'dynamic' ], true ) ) {
										$is_enabled = '';
									}
									...
									if ( 'single' === $element_object->type || 'multipleallsingle' === $element_object->type ) {
										...
										if ( isset( $builder[ 'multiple_' . $current_element . '_options_price' ][ $current_counter ] ) ) {
											...
											$_cbsmel      = $this->get_builder_element( 'multiple_' . $current_element . '_options_cbsmel', $builder, $current_builder, $current_counter, [], $current_element, '', $element_uniqueid );
											...
											foreach ( $_prices as $_n => $_price ) {
												$use_enabled = true;
												if ( 'radiobuttons' === $element_object->element_name ) {
													$use_enabled = apply_filters( 'wc_epo_hide_radiobuttons_disabled_choices', true );
												} elseif ( 'checkboxes' === $element_object->element_name ) {
													$use_enabled = apply_filters( 'wc_epo_hide_checkboxes_disabled_choices', true );
												}
												if ( $use_enabled && isset( $_enabled[ $_n ] ) && ( '0' === $_enabled[ $_n ] || '' === $_enabled[ $_n ] ) ) {
													...
													if ( isset( $_cbsmel ) && is_array( $_cbsmel ) ) {
														unset( $_cbsmel[ $_n ] );
													}
									...
									if ( 'variations' !== $current_element && $sections_repeater ) {
										$repeater              = '';
										$repeater_quantity     = '';
										$repeater_min_rows     = '';
										$repeater_max_rows     = '';
										...
										$this->cpf_single[ $element_uniqueid ] =
											array_merge(
												...
												[
													...
													'cenabled' => ( false !== $_enabled ) ? $_enabled : '',
													...
													'is_cbsmel_multiple' => isset( $_cbsmel ) ? $_cbsmel : [],
													...
													'cbsmel' => $this->get_builder_element( $_prefix . 'cbsmel', $builder, $current_builder, $current_counter, '', $current_element, '', $element_uniqueid ),
