...
final class THEMECOMPLETE_EPO_ADMIN_BUILDER_Base {
	...
	public function add_setting_validation1( $name = '' ) {
		...
		if ( 'textfield' === $name ) {
			array_splice(
				...
				[
					[
						'text'  => esc_html__( 'Password', 'woocommerce-tm-extra-product-options' ),
						'value' => 'password',
					],
				]
			);
	...
	public function add_setting_enabled( $name = '' ) {
		return [
			'id'               => $name . '_enabled',
			'message0x0_class' => 'tm-epo-enabled-wrapper',
			'wpmldisable'      => 1,
			'default'          => '1',
			'type'             => 'checkbox',
			'tags'             => [
				'value' => '1',
				'class' => 'is_enabled',
				'id'    => 'builder_' . $name . '_enabled',
				'name'  => 'tm_meta[tmfbuilder][' . $name . '_enabled][]',
			],
			'extra'            => '<span class="element-cannot-be-enabled tm-hidden">' . esc_html__( 'This element cannot be enabled because it is not supported in a repeater section.', 'woocommerce-tm-extra-product-options' ) . '</span>',
	...
	public function add_setting_price_type( $name = '' ) {
		...
		$options = [
			...
			[
				'text'  => esc_html__( 'Fixed amount (after free characters)', 'woocommerce-tm-extra-product-options' ),
				'value' => 'fixednon',
			],
			[
			...
			[
				'text'  => esc_html__( 'Price per word (after free characters)', 'woocommerce-tm-extra-product-options' ),
				'value' => 'wordnon',
			],
			[
				'text'  => esc_html__( 'Percent of the original price per word (after free characters)', 'woocommerce-tm-extra-product-options' ),
			...
			[
				'text'  => esc_html__( 'Price per character (after free characters)', 'woocommerce-tm-extra-product-options' ),
				'value' => 'charnon',
			],
			[
				'text'  => esc_html__( 'Price per character (after free characters and no spaces)', 'woocommerce-tm-extra-product-options' ),
			...
			[
				'text'  => esc_html__( 'Percent of the original price per character (after free characters)', 'woocommerce-tm-extra-product-options' ),
				'value' => 'charpercentnon',
			],
			[
				'text'  => esc_html__( 'Percent of the original price per character (after free characters and no spaces)', 'woocommerce-tm-extra-product-options' ),
		...
		if ( empty( $this->noecho ) ) {
			...
			if ( $show_buttons ) {
				if ( $wpml_is_original_product ) {
					...
					echo '<i class="tmicon tcfa tcfa-clone"></i>';
					echo '</li>';
				}
			}
			if ( $wpml_is_original_product ) {
			...
			if ( $show_buttons && $wpml_is_original_product ) {
				...
				// This element is div since firefox drag doen't work on button element when it doesn't have text.
				echo '<div class="tmicon tcfa tcfa-grip-vertical move"></div>';
				echo '</div>';
			}
			...
			echo '<div class="tm-for-bitem tm-label-desc' . ( '' !== $args['internal_name'] ? ' tc-has-value' : ' tc-empty-value' ) . '" data-element="' . esc_attr( $args['element'] ) . '">';
			echo '<div class="tm-internal-label" spellcheck="false" contenteditable="true" data-value="' . esc_attr( $args['label'] ) . '">' . ( '' !== $args['internal_name'] ? esc_html( $args['internal_name'] ) : esc_attr( $args['label'] ) ) . '</div>';
			if ( ! empty( $args['is_template'] ) ) {
				echo '<input type="hidden" value="' . esc_attr( $args['internal_name'] ) . '" name="tm_meta[tmfbuilder][' . esc_attr( $args['element'] ) . '_internal_name][]" class="t tm-internal-name">';
			}
			echo '</div>';
			...
			echo '</div>';
			if ( $wpml_is_original_product ) {
				echo '<div class="tm-label-actions">';
				echo '<button type="button" class="tmicon tcfa tcfa-ellipsis-vertical actions"></button>';
				echo '</div>';
			}
			echo '</div>';
	...
	private function section_elements_template( $args = [] ) {
		...
		if ( empty( $this->noecho ) ) {
			...
			if ( $show_buttons ) {
				if ( $wpml_is_original_product ) {
					echo '<div class="tmicon tcfa tcfa-grip-vertical move"></div>';
					echo '<div class="tmicon tcfa tcfa-times delete"></div>';
				}
				echo '<div class="tmicon tcfa tcfa-caret-down fold"></div>';
				$section_default_name = esc_html__( 'Section', 'woocommerce-tm-extra-product-options' );
				echo '<div class="tm-for-section tm-label-desc' . ( '' !== $sections_internal_name ? ' tc-has-value' : ' tc-empty-value' ) . ' display-flex">';
				echo '<div class="tm-internal-label" spellcheck="false" contenteditable="true" data-value="' . esc_html( $section_default_name ) . '">' . ( '' !== $sections_internal_name ? esc_html( (string) $sections_internal_name ) : esc_html( $section_default_name ) ) . '</div>';
				echo '</div>';
			}
			if ( ! empty( $args['is_template'] ) ) {
				echo '<input type="hidden" value="' . esc_attr( (string) $sections_internal_name ) . '" name="tm_meta[tmfbuilder][sections_internal_name][]" class="t tm-internal-name">';
			}
			echo '</div>';
		}
	}
	...
	public function ditems() {
		echo '<div class="builder-drag">';
		echo '<div class="builder-drag-elements">';
		foreach ( THEMECOMPLETE_EPO_BUILDER()->get_elements() as $element => $settings ) {
			...
			if ( $settings->show_on_backend ) {
				...
				echo '<div data-element="' . esc_attr( $element ) . '" class="ditem element-' . esc_attr( $element ) . '">'
								. '<div class="tm-label">'
								. '<div class="tm-icon-wrap"><i class="tmfa tcfa ' . esc_attr( $settings->icon ) . '"></i></div> '
								. '<div class="tm-name-wrap"><span class="tm-element-name">' . esc_attr( $settings->name ) . '</span>'
								. '<span class="tm-element-description">' . esc_html( $settings->description ) . '</span></div>'
								. '</div></div>';
			}
		}
		echo '</div>';
	...
	public function builder_sub_options( $args = [] ) {
		...
		foreach ( $options['title'] as $ar => $el ) {
			...
			if ( ! isset( $options['cbsmel'][ $ar ] ) ) {
				$options['cbsmel'][ $ar ] = $original_o['cbsmel']['default'];
			}
			foreach ( $_extra_options as $__key => $__name ) {
				if ( ( is_array( $__name['type'] ) && in_array( $type, $__name['type'], true ) ) || $__name['type'] === $type ) {
					$_extra_name = $__name['name'];
			...
			if ( empty( $return_js ) ) {
				...
				if ( 'multiple_checkboxes_options' === $name || 'multiple_radiobuttons_options' === $name ) {
					echo '<div class="tc-cell tc-col-12 tc-cell-cbsmel"><span class="tm-choice-price-wrap fullwidth"><span class="tm-choice-cl">' . esc_html__( 'Show Button Selector', 'woocommerce-tm-extra-product-options' ) . '</span>';
					THEMECOMPLETE_EPO_HTML()->create_field( $o['cbsmel'], 1 );
					echo '</span></div>';
				}
				if ( 'multiple_selectboxmultiple_options' !== $name ) {
				...
				if ( 'multiple_checkboxes_options' === $name || 'multiple_radiobuttons_options' === $name ) {
					$js_object[ $d_counter ][] = [
						'id'      => $name . '_cbsmel',
						'default' => (string) $o['cbsmel']['default'],
						'checked' => (string) $o['cbsmel']['default'] === (string) $o['cbsmel']['tags']['value'],
						'type'    => 'checkbox',
						'tags'    => [
							'name'  => 'tm_meta[tmfbuilder][' . $name . '_cbsmel][' . ( is_null( $counter ) ? 0 : $counter ) . '][]',
							'value' => $o['cbsmel']['tags']['value'],
						],
					];
				}
