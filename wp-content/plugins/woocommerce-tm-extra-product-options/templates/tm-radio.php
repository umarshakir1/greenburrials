...
if ( isset( $li_class, $label_to_display, $element_id, $replacement_mode, $border_type, $value, $name, $fieldtype, $use, $rules, $original_rules, $rules_type, $url, $image, $imagec, $imagep, $imagel, $image_variations, $checked, $swatch_position, $swatch_class ) ) :
	...
	$is_disabled = ! apply_filters( 'wc_epo_hide_radiobuttons_disabled_choices', true ) && isset( $tm_element_settings['cenabled'] ) && isset( $tm_element_settings['cenabled'][ $choice_counter ] ) && '' === $tm_element_settings['cenabled'][ $choice_counter ];

	if ( $is_disabled ) {
		$liclass .= ' tc-has-force-disabled';
	}
...
<li class="<?php echo esc_attr( $liclass ); ?>"><div class="tmcp-field-wrap-inner">
	...
	<?php else : ?>
		...
		if ( $is_disabled ) {
			$input_args['tags']['disabled'] = true;
			$input_args['tags']['class']    = $input_args['tags']['class'] . ' tc-force-disabled ignore';
		}
		...
		if ( isset( $label_mode ) && ! empty( $label_mode ) && 'text' !== $label_mode ) {
			...
			$is_cbsmel_multiple = isset( $tm_element_settings ) && isset( $tm_element_settings['is_cbsmel_multiple'] ) && isset( $field_counter ) && ! empty( $tm_element_settings['is_cbsmel_multiple'][ $field_counter ] );
			$cbsmel_button_text = apply_filters( 'wc_button_selector_text', ! empty( THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_cbsmel_button_text' ) ) ? THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_cbsmel_button_text' ) : esc_html__( 'Select choice', 'woocommerce-tm-extra-product-options' ) );
			switch ( $label_mode ) {
				case 'images':
				case 'startimages':
				case 'endimages':
			...
			if ( $is_cbsmel_multiple && 'none' !== $label_mode && 'text' !== $label_mode ) {
				echo '<button type="button" class="tc-choice-button-selector">' . esc_html( $cbsmel_button_text ) . '</button>';
			}
		}
