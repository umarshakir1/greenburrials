 ...
 * @param boolean $decode UTF-8 decode the string or not.
 * @return int
 */
function tc_strlen( $str = '', $strip = false, $decode = true ) {
...
if ( ! function_exists( 'mb_strpos' ) ) {
	 ...
	 * @return int|false
	 */
	function mb_strpos_iconv_strpos( $haystack, $needle, $offset = 0, $encoding = null ) {
		if ( null === $encoding ) {
			$encoding = 'UTF-8';
		}
	 ...
	 * @return int|false
	 */
	function mb_strpos_php( $haystack, $needle, $offset = 0, $encoding = null ) {
		if ( null === $encoding ) {
			$encoding = 'UTF-8';
		}
		...
		// Convert UTF-8 strings into an array of characters.
		$haystack_array = themecomplete_utf8_to_array( $haystack );
		$needle_array   = themecomplete_utf8_to_array( $needle );

		$haystack_slice = array_slice( $haystack_array, $offset );

		// Search for the needle in the haystack.
		$pos = strpos( implode( '', $haystack_slice ), implode( '', $needle_array ) );

		return false === $pos ? false : $pos + $offset;
	}

	/**
	 * Converts a UTF-8 encoded string into an array of its characters.
	 *
	 * @param string $str The UTF-8 encoded string.
	 *
	 * @return array An array where each element is a single UTF-8 character.
	 */
	function themecomplete_utf8_to_array( $str ) {
		$result = [];
		$len    = strlen( $str );
		for ( $i = 0; $i < $len; $i++ ) {
			$byte = ord( $str[ $i ] );
	...
	function mb_strpos( $haystack, $needle, $offset = 0, $encoding = null ) {
		if ( function_exists( 'iconv_strpos' ) ) {
			return mb_strpos_iconv_strpos( $haystack, $needle, $offset, $encoding );
		}

		return mb_strpos_php( $haystack, $needle, $offset, $encoding );
	}
}
...
if ( ! function_exists( 'themecomplete_get_attributes' ) ) {
	...
	/**
	 * Retrieves product attributes in a consistent format
	 *
	 * @param mixed $post_id Product ID or product object.
	 * @param bool  $return_array Whether to return raw attributes array without processing.
	 * @return mixed Array of product attributes or false on failure
	 */
	function themecomplete_get_attributes( $post_id, $return_array = false ) {
		if ( $return_array ) {
		...
		try {
			...
			if ( is_array( $attributes ) || $attributes instanceof ArrayAccess ) {
				foreach ( $attributes as $name => $attribute ) {
					if ( is_object( $attribute ) && is_callable( [ $attribute, 'get_data' ] ) ) {
						$processed_attributes[ $name ] = $attribute->get_data();
					}
				}
			}
