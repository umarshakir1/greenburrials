...
final class THEMECOMPLETE_EPO_HELPER_Base {
	...
	/**
	 * Checks if a string matches the format: 14 hex chars, dot, 7 hex chars (e.g. "6872cb7e558c27.86a9b41")
	 *
	 * @param string $input The string to validate.
	 * @return bool Returns true if valid, false otherwise.
	 */
	public function is_valid_id_format( $input = '' ) {
		return preg_match( '/^[a-f0-9]{14}\.[a-f0-9]{7}$/i', $input ) === 1;
	}
	...
	public function recreate_element_ids( $meta = [], $new_ids = false ) {
		...
		if ( isset( $builder ) ) {
			...
			if ( false === $new_ids ) {
				...
				foreach ( $ids as $idx => $idelement ) {
					foreach ( $idelement as $idy => $id ) {
						$new_ids[ $id ] = $this->is_valid_id_format( $id )
							? THEMECOMPLETE_EPO_HELPER()->tm_uniqid()
							: $id;
					}
				}
			}
	...
	/**
	 * Converts a raw posted date string into the WordPress localized date format.
	 *
	 * This method attempts to parse a date string (e.g., from form submissions) using a
	 * provided format, normalize it with optional timezone adjustments, and return it
	 * formatted according to the site's "Date Format" setting in WordPress.
	 *
	 * Behavior:
	 * - Trims the posted date string and parses it with the format definition retrieved
	 *   from THEMECOMPLETE_EPO()->get_date_format().
	 * - Adjusts parsing for RTL locales via the `wc_epo_display_rtl` filter.
	 * - Supports optional timezone handling based on
	 *   THEMECOMPLETE_EPO_DATA_STORE()->get( 'tm_epo_global_date_timezone' ).
	 * - Falls back to attempting reversed date components if parsing fails.
	 * - If parsing succeeds, returns the date formatted via date_i18n() using
	 *   get_option( 'date_format' ).
	 * - If parsing fails entirely, returns the original posted date string.
	 *
	 * @param string            $posted_date Raw posted date string (e.g. "19/09/2025").
	 * @param string|false|null $format      Optional format identifier used by
	 *                                       THEMECOMPLETE_EPO()->get_date_format().
	 *                                       Default false.
	 *
	 * @return string Localized date string formatted according to the WordPress
	 *                date format, or the original posted date string if conversion fails.
	 *
	 * @since 7.5.3
	 *
	 * @uses THEMECOMPLETE_EPO()->get_date_format()
	 * @uses THEMECOMPLETE_EPO_DATA_STORE()->get()
	 * @uses date_i18n()
	 * @uses get_option( 'date_format' )
	 *
	 * @filter wc_epo_display_rtl Adjusts date parsing for right-to-left locales.
	 */
	public function convert_posted_date_to_wp_date( $posted_date = '', $format = false ) {
		if ( false !== $format && null !== $format ) {
			...
			if ( '' !== $posted_date ) {
				...
				if ( false === $date || ! empty( $date_errors['error_count'] ) ) {
					$parts = explode( '/', $posted_date );
					if ( 3 === count( $parts ) ) {
						// Reverse parts: year/month/day → day/month/year.
				...
				$date_errors = DateTime::getLastErrors();
				if ( false !== $date && empty( $date_errors['error_count'] ) ) {
					$posted_date = date_i18n( get_option( 'date_format' ), $date->getTimestamp() );
				}
			}
		}
		return $posted_date;
